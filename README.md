...
You can install the tools in this repository with pip:
```
pip install git+https://github.com/braingeneers/human_hippocampus.git
```
This command can take a bit of time to run.

Alternatively, one can clone the repository to a specified location and then call:
```
pip install -e [PATH_TO_CLONED_RESPOSITORY]
```
This second option makes it easier for the user to edit the analysis scripts to suit their own needs.

The package and associated modules can then be imported into a python script with:
```
import human_hip.raw_data
import human_hip.spike_data
import human_hip.animate
import human_hip.histology
```
See the tutorials (`tutorials/`) and source code (`src/`) to see what functions are included in each module and how to use them.

The GitHub Codespace makes use of Python 3.10.0 - utilizing this version of Python with one’s local installation of the package ensures compatibility. Additional dependencies may need to be installed in order to properly run all functions in the toolbox.
...
- **tutorials/:** This folder contains a series of Jupyter Notebooks highlighting how to perform basic analyses and create various animations.
